...
The in-toto Statement (:mod:`sectum.evidence.intoto`) is a tool-agnostic view of an
...
in :mod:`sectum.evidence.sigstore` behind the optional ``[sigstore]`` extra; an
...
from sectum.evidence.intoto import to_in_toto_statement, verify_in_toto_statement
...
def envelope_statement(envelope: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Raises :class:`~sectum.spec.EvidenceError` if the envelope is malformed or its
...
def verify_dsse_envelope(envelope: Mapping[str, Any], pack: EvidencePack) -> None:
    ...
    additional concern of :mod:`sectum.evidence.sigstore`.
    """
