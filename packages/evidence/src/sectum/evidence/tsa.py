...
The default :class:`~sectum.evidence.chain.LocalTimestamper` records a wall-clock
...
from sectum.spec import EvidenceError
...
def _builtin_cert(filename: str) -> bytes:
    ...
    return resources.files("sectum.evidence").joinpath("_tsa", filename).read_bytes()


class Rfc3161Timestamper:
    """A :class:`~sectum.evidence.chain.Timestamper` backed by an RFC 3161 TSA.
...
def verify_rfc3161_token(
    ...
    *,
    ...
    :class:`~sectum.spec.EvidenceError` if the token is malformed, the chain
