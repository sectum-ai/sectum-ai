...
After the run digest is timestamped (the RFC 3161 path in :mod:`sectum.evidence.tsa`),
...
def _builtin_keyring() -> dict[str, bytes]:
    ...
    anchor = resources.files("sectum.evidence").joinpath("_rekor")
...
def verify_rekor_proof(
    ...
    *,
    ...
    private instance. Raises :class:`~sectum.spec.EvidenceError` on any failure.
    """
    try:
