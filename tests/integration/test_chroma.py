...
def test_chroma_user_scoped_isolates_users() -> None:
    if not _server_ready():
        pytest.skip("Chroma backend not reachable")
    scoped = ChromaVectorStore(_HOST, _PORT, _embed, prefix="sectum-it-us", user_scoped=True)
...
def test_chroma_tenant_only_store_leaks_across_users(store: ChromaVectorStore) -> None:
