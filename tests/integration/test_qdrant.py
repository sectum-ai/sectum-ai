"""Integration tests for the live Qdrant adapter.

Requires a Qdrant server (see compose.yaml) and the ``qdrant`` extra. The
``store`` fixture retries the connection briefly and skips the tests when the
...
def store() -> Iterator[QdrantVectorStore]:
    ...
    instance = QdrantVectorStore(_embed, dim=_DIM, host=_HOST, port=_PORT, prefix="sectum-it")
...
def test_qdrant_round_trips_and_isolates_tenants(store: QdrantVectorStore) -> None:
...
def test_qdrant_delete_and_list_namespaces(store: QdrantVectorStore) -> None:
    store.upsert(_TENANT_A, _documents(_TENANT_A, "a", "alpha"))
...
def test_qdrant_fetch_by_id_is_tenant_scoped(store: QdrantVectorStore) -> None:
    store.upsert(_TENANT_A, _documents(_TENANT_A, "a", "alpha"))
...
def test_qdrant_user_scoped_isolates_users() -> None:
    if not _server_ready():
        pytest.skip("Qdrant backend not reachable")
    scoped = QdrantVectorStore(
