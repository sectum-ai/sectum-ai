...
Two cross-cutting boolean knobs apply to most `fake` adapters (and the live ones
that support them; the `rag` and `agent` resolvers read neither, and passing one
there is a `ConfigError`), beyond the per-family fields the tables below highlight:
...
| `gcs` | `bucket: str` *(required)*, `project: str` *(optional)*, `prefix: str = "sectum-ai-backup"`, `no_erasure: bool = false`, `soft_delete: bool = false` | `GCSBackup` — the Google-cloud parallel of `s3`; each tenant's snapshots live under the object-name prefix `{prefix}/{tenant.hex}/` in one bucket, with the same search / purge / `no_erasure` (bucket-lock) / `soft_delete` contract. Credentials fall back to Application Default Credentials; export `STORAGE_EMULATOR_HOST` to target a local fake-gcs-server. Requires the `gcs` extra. |
...
| `semantic_threshold` | float in `[0.0, 1.0]` \| `auto` | `0.62` | The similarity gate before the judge. A number is used verbatim. `auto` resolves to the calibrated per-model preset for the embedder above (see below), falling back to `0.62` with a logged warning for an unknown model. Raise it (or use `auto`) once a real embedding model is configured — a stronger model surfaces more (the engineering spec, section 7). **Bounded to the range a cosine similarity can occupy**: a value above `1.0` is unsatisfiable and would switch the semantic detector off entirely — no candidate reaches the judge, a paraphrased cross-tenant leak is not recorded even as unverified, and the run still signs a pack asserting isolation. Out-of-range values (including `nan` and `inf`) are rejected at config load. `0.0` is legal and admits every candidate to the judge. |
...
`SectumConfig`, `ScenarioConfig`, `AdapterConfig`, `EvidenceConfig`,
`SecurityConfig` and `DetectionConfig` (the last two have their own sections
above) — and the
