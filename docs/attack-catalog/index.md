...
A3 probes predate it and are driven directly by the CLI), and each ships with
unit tests and a deterministic in-memory fake adapter. The catalog grows by phase;
...
| [7 — Agent tool-call hijacking](class-07-agent-tool-hijack.md) | `agent-tool-hijack`, `agent-framework-hijack` | `AML.T0024`, `AML.T0053`; `AML.T0051.001` (`agent-tool-hijack`'s injection sub-probe only) | MCP, Agent framework |
