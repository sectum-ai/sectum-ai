...
> The action was added after `v0.1.1`, so reference `@main` until a release tag
> includes it, then pin to that tag (or a commit SHA) for reproducible runs.

## Inputs

| Input | Default | Description |
|---|---|---|
| `version` | `0.1.1` | `sectum-ai` version to install from PyPI. Pin for reproducibility; leave empty for the latest release. |
