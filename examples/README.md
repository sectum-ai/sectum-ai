...
| [`rag-pipeline-bleed/`](rag-pipeline-bleed/) | Class 2 | The same organic entity-bleed measured at the end of a RAG pipeline (retriever + generator) rather than at the vector index. The walkthrough runs the built-in fake pipeline; point it at a live `rag` adapter to measure yours. |
