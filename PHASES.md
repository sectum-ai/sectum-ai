...
- **P5** — multi-embedding-model config + a live-provider sweep, which also unlocks the Phase-5 *full-CLI* model-swap regression E2E. *In progress:* the provider-agnostic embedding interface + real-provider sweep have landed; the remaining CLI `embedding_models` config wiring closes the Phase-5 criterion above.
